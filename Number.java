...
public class Number {
  protected Button button0, button1, button2, button3, button4, button5, button6, button7, button8,
      button9, buttonDelete;
  private Font boldFont3;
  ...
  /**
   * Режим игры(0-создание числа первым игроком,1- создание числа вторым игроком, 2- игрок против
   * компьютера(легкий уровень),3- игрок против компьютера(средний уровень), 4-игрок против
   * компьютера(тяжелый уровень)
   */
  private int mode;
  ...
  public Number(Display display, Shell shell, int i) {
    ...
    this.mode = i;
    boldFont3 = new Font(shell.getDisplay(), new FontData("Arial", 12, SWT.NORMAL));
  }
  ...
  public void Show() {
    ...
    buttonOkey.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
      public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
        if (digitsInLabel == 4) {
          if (mode == 0) {
            string1 = enteredNumberLabel.getText();
            digitsInLabel = 0;
            Number number = new Number(display, shell, 1);
            number.Show();
          } else if (mode == 1) {
            string2 = enteredNumberLabel.getText();
            digitsInLabel = 0;
            Players players = new Players(display, shell, string1, string2);
            players.Show();
          } else {
            string1 = enteredNumberLabel.getText();
            string2 = CreateBotNumber();
            digitsInLabel = 0;
            ComputerPlay computer = new ComputerPlay(display, shell, string1, string2, mode);
            computer.Show();
          }
        }
      }
    ...
    backToMenu.setBackground(shell.getDisplay().getSystemColor(SWT.COLOR_DARK_MAGENTA));
    backToMenu.setLayoutData(griddataButtonBackToMenu);
    backToMenu.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
      public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
        digitsInLabel = 0;
  ...
  private void deleteShow() {
    ...
    Font boldFont1 =
        new Font(enteredNumberLabel.getDisplay(), new FontData("Arial", 40, SWT.NORMAL));
  ...
  public static String CreateBotNumber() {
    ...
    do {
      ...
      if (botNumber.contains(digit) == false) {
        botNumber += String.valueOf(digit);
        i++;
      }
    } while (i < 4);
    return botNumber;
